    ...
    public Page<Instance> findInstancesByNamespace(String appId, String clusterName, String
        ...
        if (instanceConfigs.hasContent()) {
            Set<Long> instanceIds =
                    FluentIterable.from(instanceConfigs.getContent()).transform(new Function<InstanceConfig, Long>() {
                        @Override
                        public Long apply(InstanceConfig instanceConfig) {
                            return instanceConfig.getInstanceId();
                        }
                    }).toSet();
//              instanceConfigs.getContent().stream().map
//          (InstanceConfig::getInstanceId).collect(Collectors.toSet());
    ...
    public Page<Instance> findInstancesByNamespaceAndInstanceAppId(String instanceAppId, String
        ...
        if (instanceIdResult.hasContent()) {
            Set<Long> instanceIds =
                    FluentIterable.from(instanceIdResult.getContent()).transform(new Function<Object, Long>() {
                        @Override
            ...
            //todo 此处转换时的数组
//              instanceIdResult.getContent().stream().map((Object o) -> {
//        if (o == null) {
//          return null;
//        }
//
